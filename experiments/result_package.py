...
def build_result_package(
    *,
    ...
    sampling_contract = analysis_contract.get("sampling_window_s")
    sampling_window_s = (
        sampling_contract.get("value")
        if isinstance(sampling_contract, Mapping)
        else DEFAULT_SAMPLING_WINDOW_S
    )
    ...
    with zipfile.ZipFile(archive, "w", compression=zipfile.ZIP_DEFLATED) as bundle:
        ...
        bundle.writestr(
            prefix + "velocity_vector_field.json",
            json.dumps(velocity_field, ensure_ascii=False, indent=2),
        )
        bundle.writestr(

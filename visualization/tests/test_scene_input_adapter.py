...
class SceneInputAdapterTests(unittest.TestCase):
    def test_json_map_loads_into_static_visualizer_snapshot(self) -> None:
        ...
        self.assertEqual(snapshot["grid_width"], 20)
        self.assertEqual(snapshot["grid_height"], 20)
        self.assertEqual(snapshot["cell_size"], 0.5)
        self.assertTrue(snapshot["exits"])
    ...
    def test_d_test_population_output_is_validated(self) -> None:
        view = load_population_output(
            ROOT / "docs" / "d_week3" / "examples" / "d_test_population.json",
            source_id_base=1,
        )
        self.assertEqual(view.metadata["source"], "D_TEST")
        self.assertEqual(len(view.persons), 40)
        self.assertEqual(len(view.relations), 20)

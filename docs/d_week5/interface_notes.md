...
- B 仿真：通过 `experiments.integrated_runner` 和 `EvacEngineRuntimeAdapter` 调用当前 `simulation.evac_simulation.EvacEngine.step()`，D 端不修改 B 算法。
...
- B：`EvacEngine` 当前仍会创建 Matplotlib 图窗；D 的 web bridge 已在服务端启用 headless 后端以避免线程 GUI 报错，但是否保留 GUI 绘制应由 B 确认。

...
8. 新增 D-only 场景输入适配器，接入 A 的 JSON/CSV Grid 和 C 的 YAML SceneConfig 参数；
...
1. B 尚未确认唯一正式仿真入口；
2. B 尚未提供公开的逐步快照、撤离状态、冲突和出口切换事件；
3. A 的 JSON/CSV 已能进入 D 地图预览，但地图烟源和出口元数据尚未组装为 B 的 `ScenarioConfig`；
4. C 的 YAML 配置可读取，但当前交付没有 persons/relations 生成接口；
5. C 的人员编号、比例分配和关系强度口径仍需团队确认；
...
2. 收到 C 的 persons/relations 正式输出后，将 YAML 配置接入场景工厂；
3. 根据 B 确认的正式入口替换临时 mock 入口；
4. 接入公开的 `heading`、`risk`、`dose`、拥堵和事件字段；
5. 使用更大场景检查动画性能、日志规模和异常处理；
6. 开始计算第一批真实疏散指标。

## 6. 对其他成员接口的影响

D 没有修改 A、B、C 的代码或模型逻辑。
